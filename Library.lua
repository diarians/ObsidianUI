...
local BaseURL = "https://v1zhub.live/obsidian/"
...
local Library = {
    ...
    IsLightTheme = false,
    Scheme = {
        BackgroundColor = Color3.fromRGB(10, 11, 14),
        MainColor = Color3.fromRGB(18, 19, 23),
        AccentColor = Color3.fromRGB(125, 85, 255),
        OutlineColor = Color3.fromRGB(30, 31, 36),
...
local FetchIcons, Icons = pcall(function()
    return (loadstring(
        game:HttpGet("https://v1zhub.live/obsidian/lucide-source.lua")
    ) :: () -> IconModule)()
end)
...
function Library:MakeDraggable(UI: GuiObject, DragFrame: GuiObject, IgnoreToggled: boolean?, IsMainWindow: boolean?)
    if not UI or not DragFrame then return end
    local StartPos
    local FramePos
    ...
    InputChanged = UserInputService.InputChanged:Connect(function(Input: InputObject)
        ...
        if Dragging and IsHoverInput(Input) then
            ...
            DragTween = TweenService:Create(UI, TweenInfo.new(0.05, Enum.EasingStyle.Linear), {Position = NewPos})
            DragTween:Play()
        end
    end)
...
-- Soft neon accent glow: stacked accent UIStrokes (thin+brighter -> thick+fainter) that
-- fake a soft outer glow. No animation; "AccentColor" scheme key keeps it synced to accent.
Library.Glows = {}
function Library:AddGlow(Frame: GuiObject)
    if not Frame then return end
    -- Smooth soft glow: many thin accent strokes, gradually increasing thickness +
    -- transparency, so they BLEND into one soft halo. (3 far-spaced layers rendered as
    -- distinct lines instead of a glow.) Inner = brighter, outer = fades out.
    local LAYERS = 10
    for i = 1, LAYERS do
        local frac = i / LAYERS
        local Glow = New("UIStroke", {
            ...
            Color = "AccentColor",
            Thickness = frac * 8,
            Transparency = 0.74 + 0.22 * frac,
            ZIndex = 0,
            Parent = Frame,
        })
        table.insert(Library.Glows, Glow)
    end
end
...
--// TVL Watermark (FPS + ping + inline icon) — call once from a script, self-updating \\--
function Library:CreateWatermark(...)
    local Params = select(1, ...)
    local Title, Prefix, IconAsset
    if typeof(Params) == "table" then
        Title = Params.Title or "Watermark"
        Prefix = Params.Prefix or (Title .. "       |  ")
        IconAsset = Params.Icon
    else
        Title = tostring(Params or "Watermark")
        Prefix = select(2, ...) or (Title .. "       |  ")
        IconAsset = select(3, ...)
    end

    local RichPrefix = Prefix
    if typeof(Title) == "string" and Title ~= "" and typeof(Prefix) == "string" and Prefix:sub(1, #Title) == Title then
        RichPrefix = ('<font transparency="1">%s</font>'):format(Title) .. Prefix:sub(#Title + 1)
    end
    ...
    local DL = Library:AddDraggableLabel(Title)
    local Watermark = { DraggableLabel = DL, Destroyed = false }

    local _richLabel = nil
    local _container = nil
    local _icon = nil
    local _cachedIconTextWidth = 40
    local _cachedIconTextSize = nil
    local _cachedIconFont = nil

    local function resolveLabel()
        local container = DL and (DL.Label or DL.Frame or DL.Instance)
        if typeof(container) ~= "Instance" then return nil, nil end
        if container:IsA("TextLabel") then
            container.RichText = true
            return container, container
        end
        for _, v in ipairs(container:GetDescendants()) do
            if v:IsA("TextLabel") then v.RichText = true; return container, v end
        end
        return container, nil
    end

    local function ensureIcon(container, label)
        if not IconAsset or tostring(IconAsset) == "" then return end
        if typeof(container) ~= "Instance" or typeof(label) ~= "Instance" then return end
        if not (_icon and _icon.Parent == container) then
            if _icon and _icon.Parent then pcall(function() _icon:Destroy() end) end
            _icon = Instance.new("ImageLabel")
            _icon.Name = "WatermarkIcon"
            _icon.BackgroundTransparency = 1
            _icon.Image = tostring(IconAsset)
            _icon.ScaleType = Enum.ScaleType.Fit
            _icon.AnchorPoint = Vector2.new(0, 0.5)
            _icon.ZIndex = label.ZIndex + 1
            _icon.Parent = container
            _cachedIconTextSize = nil
        end
        local textSize = tonumber(label.TextSize) or 15
        local textFont = label.Font or Enum.Font.Gotham
        if textSize ~= _cachedIconTextSize or textFont ~= _cachedIconFont then
            _cachedIconTextSize = textSize
            _cachedIconFont = textFont
            pcall(function()
                _cachedIconTextWidth = TextService:GetTextSize(Title, textSize, textFont, Vector2.new(1000, textSize + 4)).X
            end)
        end
        _icon.Size = UDim2.fromOffset(textSize, textSize)
        _icon.Position = UDim2.new(0, _cachedIconTextWidth + 8, 0.5, 0)
    end

    local _brand = nil
    local function ensureBrand(container, label)
        if typeof(container) ~= "Instance" or typeof(label) ~= "Instance" then return end
        if not (_brand and _brand.Parent == container) then
            if _brand and _brand.Parent then pcall(function() _brand:Destroy() end) end
            _brand = Instance.new("TextLabel")
            _brand.Name = "WatermarkBrand"
            _brand.BackgroundTransparency = 1
            _brand.RichText = false
            _brand.Text = Title
            _brand.TextColor3 = Color3.new(1, 1, 1)
            _brand.TextXAlignment = Enum.TextXAlignment.Left
            _brand.TextYAlignment = Enum.TextYAlignment.Center
            _brand.AutomaticSize = Enum.AutomaticSize.X
            _brand.AnchorPoint = Vector2.new(0, 0.5)
            _brand.Parent = container
            pcall(function()
                local reg = Library:AddSharedGradient(_brand, { isRgb = false })
                if reg then reg.alwaysUpdate = true end
            end)
        end
        _brand.Font = label.Font
        _brand.TextSize = label.TextSize
        _brand.ZIndex = (tonumber(label.ZIndex) or 10) + 2
        _brand.Size = UDim2.fromOffset(0, label.TextSize)
        _brand.Position = UDim2.new(0, 0, 0.5, 0)
    end

    -- position top-center on next frame (still fully draggable afterwards)
    task.delay(0, function()
        pcall(function()
            local container, label = resolveLabel()
            ...
            _richLabel = label
            if label then label.TextXAlignment = Enum.TextXAlignment.Left end
            ensureIcon(container, label)
            ensureBrand(container, label)
        end)
    end)

    local _frameTimer, _frameCount, _fps = tick(), 0, 60
    local _lastUpdate, _cachedPing = 0, 0
    local _conn
    _conn = RunService.RenderStepped:Connect(function()
        pcall(function()
            _frameCount += 1
            local now = tick()
            if (now - _frameTimer) >= 1 then _fps = _frameCount; _frameTimer = now; _frameCount = 0 end
            if (now - _lastUpdate) < 1 then return end
            _lastUpdate = now
            if not (_richLabel and _richLabel.Parent) then
                local container, label = resolveLabel()
                _container = container; _richLabel = label
            end
            ensureIcon(_container, _richLabel)
            ensureBrand(_container, _richLabel)
            _cachedPing = math.floor(Stats.Network.ServerStatsItem["Data Ping"]:GetValue())
            if _richLabel then
                local fc = _fps >= 55 and "#4ade80" or _fps >= 30 and "#facc15" or "#f87171"
                local pc = _cachedPing <= 80 and "#4ade80" or _cachedPing <= 150 and "#facc15" or "#f87171"
                _richLabel.Text = (RichPrefix .. '<font color="%s">%d FPS</font>  |  <font color="%s">%d ms</font>'):format(fc, _fps, pc, _cachedPing)
            else
                DL:SetText((Prefix .. "%d FPS  |  %d ms"):format(_fps, _cachedPing))
            end
            if DL.Destroyed and _conn then _conn:Disconnect() end
        end)
    end)
    if DL.Connections then table.insert(DL.Connections, _conn) end

    function Watermark:SetText(t) DL:SetText(t) end
    function Watermark:SetVisible(v) DL:SetVisible(v) end
    function Watermark:Destroy()
        Watermark.Destroyed = true
        if _conn then pcall(function() _conn:Disconnect() end) end
        if _icon and _icon.Parent then pcall(function() _icon:Destroy() end) end
        if _brand and _brand.Parent then pcall(function() _brand:Destroy() end) end
        pcall(function() DL:Destroy() end)
    end

    return Watermark
end

--// Accent RGB driver + reusable accent/rainbow gradient (call from any script) \\--
-- Library:SetRgbAccent(true/false [, speed])  -> cycles Library.Scheme.AccentColor through the hue
--   wheel and writes every registered AccentColor prop; exposes Library.RgbEnabled / Library.RgbHue.
--   Mirrors hue to getgenv().__tvlRgbHue / __tvlRgbConn for backward-compat readers.
function Library:SetRgbAccent(...)
    local Params = select(1, ...)
    local enabled, speed
    if typeof(Params) == "table" then
    ...
    speed = tonumber(speed) or 0.0015
    local _genv = (type(getgenv) == "function" and getgenv()) or _G

    if self._rgbConn then pcall(function() self._rgbConn:Disconnect() end); self._rgbConn = nil end
    if not enabled then
        self.RgbEnabled = false
        _genv.__tvlRgbConn = nil
        local orig = self._rgbOrigAccent
        if orig then
            self.Scheme.AccentColor = orig
            for inst, props in pairs(self.Registry) do
                if type(props) == "table" then
                    for prop, colorName in pairs(props) do
                        if colorName == "AccentColor" then pcall(function() inst[prop] = orig end) end
                    end
                end
            end
            pcall(function() local p = self.Options and self.Options.AccentColor; if p then p:SetValue(orig) end end)
        end
        return
    end

    self._rgbOrigAccent = self.Scheme.AccentColor
    self.RgbEnabled = true
    local hue, frame, bindings, rebuildAt = 0, 0, {}, 0
    local function rebuild()
        table.clear(bindings)
        for inst, props in pairs(self.Registry) do
            if type(props) == "table" then
                for prop, colorName in pairs(props) do
                    if colorName == "AccentColor" then bindings[#bindings + 1] = { inst, prop } end
                end
            end
        end
    end
    self._rgbConn = game:GetService("RunService").Heartbeat:Connect(function()
        frame = frame + 1
        if frame % 3 ~= 0 then return end
        hue = (hue + speed) % 1
        self.RgbHue = hue
        _genv.__tvlRgbHue = hue
        local col = Color3.fromHSV(hue, 1, 1)
        self.Scheme.AccentColor = col
        local now = os.clock()
        if now - rebuildAt > 2 then rebuildAt = now; rebuild() end
        for _, b in ipairs(bindings) do pcall(function() b[1][b[2]] = col end) end
        pcall(function()
            local p = self.Options and self.Options.AccentColor
            if p then p.Hue = hue; p.Sat = 1; p.Vib = 1; p:Update() end
        end)
    end)
    _genv.__tvlRgbConn = self._rgbConn
end

-- Library:AddAccentGradient(guiObject [, opts]) -> attaches a scrolling gradient that follows the
--   accent color and goes rainbow while SetRgbAccent is on. opts: Rotation, Transparency (NumberSequence),
--   Scroll (bool, default true), ScrollSpeed (default 1.8). Returns { Gradient, Destroy }.
local function _gradOnScreen(inst)
    local node = inst
    while node do
        if node:IsA("GuiObject") then
            if not node.Visible then return false end
        elseif node:IsA("LayerCollector") then
            return node.Enabled and node.Parent ~= nil
        end
        node = node.Parent
    end
    return false
end
function Library:_ensureGradLoop()
    if self._sharedGradConn then return end
    self._sharedGrads = self._sharedGrads or {}
    local RS = game:GetService("RunService")
    self._sharedGradConn = RS.Heartbeat:Connect(function()
        local grads = self._sharedGrads
        if not grads or #grads == 0 then return end
        local menuVisible = (self.Toggled and not self.Unloaded and self.ScreenGui and self.ScreenGui.Parent) and true or false
        local now = os.clock()
        local accent = (self.Scheme and self.Scheme.AccentColor) or Color3.fromRGB(40, 76, 177)
        local rgbOn = self.RgbEnabled and true or false
        local rgbHue = self.RgbHue or 0
        local secHue = (now * 0.4) % 1
        local secRgbSeq, rgbModeSeq, accentSeq16, accentSeq17
        for i = #grads, 1, -1 do
            local e = grads[i]
            if not e.gradient or not e.gradient.Parent then
                table.remove(grads, i)
            elseif e.lbl and e.lbl.Parent then
                local _upd
                if e.alwaysUpdate then
                    _upd = _gradOnScreen(e.lbl)
                else
                    _upd = menuVisible or _gradOnScreen(e.lbl)
                end
                if _upd then
                if e.lbl:IsA("ImageLabel") and e.lbl.ImageColor3 ~= Color3.new(1, 1, 1) then
                    e.lbl.ImageColor3 = Color3.new(1, 1, 1)
                end
                if e.kind == "section" and e.isRgb then
                    if not secRgbSeq then
                        secRgbSeq = ColorSequence.new({
                            ColorSequenceKeypoint.new(0,   Color3.fromHSV((secHue + 0)   % 1, 1, 1)),
                            ColorSequenceKeypoint.new(1/6, Color3.fromHSV((secHue + 1/6) % 1, 1, 1)),
                            ColorSequenceKeypoint.new(2/6, Color3.fromHSV((secHue + 2/6) % 1, 1, 1)),
                            ColorSequenceKeypoint.new(3/6, Color3.fromHSV((secHue + 3/6) % 1, 1, 1)),
                            ColorSequenceKeypoint.new(4/6, Color3.fromHSV((secHue + 4/6) % 1, 1, 1)),
                            ColorSequenceKeypoint.new(5/6, Color3.fromHSV((secHue + 5/6) % 1, 1, 1)),
                            ColorSequenceKeypoint.new(1,   Color3.fromHSV((secHue + 0)   % 1, 1, 1)),
                        })
                    end
                    e.gradient.Color = secRgbSeq
                elseif e.kind == "accent" and rgbOn then
                    if math.abs(rgbHue - (e.lastHue or -1)) > 0.005 or (e.lastHue or -1) < 0 then
                        e.lastHue = rgbHue; e.lastAccent = nil
                        if not rgbModeSeq then
                            rgbModeSeq = ColorSequence.new({
                                ColorSequenceKeypoint.new(0,   Color3.fromHSV((rgbHue + 0)   % 1, 1, 1)),
                                ColorSequenceKeypoint.new(1/6, Color3.fromHSV((rgbHue + 1/6) % 1, 1, 1)),
                                ColorSequenceKeypoint.new(2/6, Color3.fromHSV((rgbHue + 2/6) % 1, 1, 1)),
                                ColorSequenceKeypoint.new(3/6, Color3.fromHSV((rgbHue + 3/6) % 1, 1, 1)),
                                ColorSequenceKeypoint.new(4/6, Color3.fromHSV((rgbHue + 4/6) % 1, 1, 1)),
                                ColorSequenceKeypoint.new(5/6, Color3.fromHSV((rgbHue + 5/6) % 1, 1, 1)),
                                ColorSequenceKeypoint.new(1,   Color3.fromHSV((rgbHue + 0)   % 1, 1, 1)),
                            })
                        end
                        e.gradient.Color = rgbModeSeq
                    end
                else
                    e.lastHue = -1
                    if accent ~= e.lastAccent then
                        e.lastAccent = accent
                        if e.kind == "accent" then
                            if not accentSeq17 then
                                local l = Color3.new(math.min(accent.R * 1.7, 1), math.min(accent.G * 1.7, 1), math.min(accent.B * 1.7, 1))
                                accentSeq17 = ColorSequence.new({
                                    ColorSequenceKeypoint.new(0,   accent),
                                    ColorSequenceKeypoint.new(0.5, l),
                                    ColorSequenceKeypoint.new(1,   accent),
                                })
                            end
                            e.gradient.Color = accentSeq17
                        else
                            if not accentSeq16 then
                                local l = Color3.new(math.min(accent.R * 1.6, 1), math.min(accent.G * 1.6, 1), math.min(accent.B * 1.6, 1))
                                accentSeq16 = ColorSequence.new({
                                    ColorSequenceKeypoint.new(0,   accent),
                                    ColorSequenceKeypoint.new(0.5, l),
                                    ColorSequenceKeypoint.new(1,   accent),
                                })
                            end
                            e.gradient.Color = accentSeq16
                        end
                    end
                end
                if e.scroll ~= false then
                    local ox = ((now * (e.scrollSpeed or 1.8)) % 2) - 1
                    if ox ~= e.lastOff then e.lastOff = ox; e.gradient.Offset = Vector2.new(ox, 0) end
                end
                end
            end
        end
...
function Library:AddAccentGradient(obj, opts)
    if typeof(obj) ~= "Instance" then return end
    opts = opts or {}
    self._sharedGrads = self._sharedGrads or {}
    local grad = Instance.new("UIGradient")
    grad.Rotation = tonumber(opts.Rotation) or 0
    if opts.Transparency then grad.Transparency = opts.Transparency end
    grad.Parent = obj
    local entry = {
        lbl = obj, gradient = grad, kind = "accent",
        scroll = opts.Scroll ~= false,
        scrollSpeed = tonumber(opts.ScrollSpeed) or 1.8,
        lastAccent = nil, lastHue = -1, lastOff = nil,
    }
    table.insert(self._sharedGrads, entry)
    self:_ensureGradLoop()
    return {
        Gradient = grad,
        Destroy = function()
            for k = #self._sharedGrads, 1, -1 do
                if self._sharedGrads[k] == entry then table.remove(self._sharedGrads, k); break end
            end
...
function Library:AddSharedGradient(obj, opts)
    if typeof(obj) ~= "Instance" then return end
    opts = opts or {}
    self._sharedGrads = self._sharedGrads or {}
    ...
    gradient.Rotation = tonumber(opts.Rotation) or 0
    gradient.Parent = obj
    local entry = {
        lbl = obj, gradient = gradient, kind = "section",
        isRgb = opts.isRgb and true or false,
        scroll = true, scrollSpeed = 1.8,
        lastAccent = nil, lastHue = -1, lastOff = nil,
    }
    table.insert(self._sharedGrads, entry)
    self:_ensureGradLoop()
...
function Library:AddContextMenu(
    ...
    function Table:Open()
        ...
        if Menu.AutomaticSize == Enum.AutomaticSize.None then
            if Table._CloseTween then
                pcall(function() Table._CloseTween:Cancel() end)
                Table._CloseTween = nil
            end
            Menu.ClipsDescendants = true
            Menu.Size = UDim2.new(TargetSize.X.Scale, TargetSize.X.Offset, 0, 0)
            Menu.Visible = true
            TweenService:Create(Menu, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = TargetSize,
            }):Play()
        else
            Menu.Visible = true
    ...
    function Table:Close()
        ...
        if Menu.AutomaticSize == Enum.AutomaticSize.None and Menu.Visible then
            local CurSize = Menu.Size
            local Tween = TweenService:Create(Menu, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
                Size = UDim2.new(CurSize.X.Scale, CurSize.X.Offset, 0, 0),
            })
            Table._CloseTween = Tween
...
function Library:AddTooltip(InfoStr: string, DisabledInfoStr: string, HoverInstance: GuiObject)
    ...
    local function DoHover()
        ...
        task.wait(0.25)
        if CurrentHoverInstance ~= HoverInstance or CurrentMenu or Library.ActiveDialog then return end
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        do
            ...
            local _kbGrad = Instance.new("UIGradient")
            _kbGrad.Enabled = false
            _kbGrad.Parent = Label

            local function _startKbGrad()
                if not Library._kbGradTargets then Library._kbGradTargets = {} end
                table.insert(Library._kbGradTargets, _kbGrad)
                pcall(function() _kbGrad.Enabled = true end)
                if not Library._kbGradConn then
                    Library._kbGradTime = Library._kbGradTime or 0
                    Library._kbGradConn = RunService.Heartbeat:Connect(function(dt)
                        Library._kbGradTime = (Library._kbGradTime + dt * 3.0) % (math.pi * 2)
                        local t = Library._kbGradTime
                        local steps = 6
                        local kps = table.create(steps + 1)
                        for i = 0, steps do
                            local pos = i / steps
                            local v = math.max(0, math.min(1, 0.65 + math.sin(pos * math.pi * 2 + t) * 0.35))
                            kps[i + 1] = ColorSequenceKeypoint.new(pos, Color3.new(v, v, v))
                        end
                        local seq = ColorSequence.new(kps)
                        for i = #Library._kbGradTargets, 1, -1 do
                            local g = Library._kbGradTargets[i]
                            if not g.Parent then
                                table.remove(Library._kbGradTargets, i)
                            else
                                pcall(function() g.Color = seq end)
                            end
                        end
                        if #Library._kbGradTargets == 0 then
                            Library._kbGradConn:Disconnect()
                            Library._kbGradConn = nil
                        end
                    end)
                end
            end
            local function _stopKbGrad()
                pcall(function() _kbGrad.Enabled = false end)
                if Library._kbGradTargets then
                    for i = #Library._kbGradTargets, 1, -1 do
                        if Library._kbGradTargets[i] == _kbGrad then
                            table.remove(Library._kbGradTargets, i)
                        end
                    end
                    if #Library._kbGradTargets == 0 and Library._kbGradConn then
                        Library._kbGradConn:Disconnect()
                        Library._kbGradConn = nil
                    end
                end
            ...
            function KeybindsToggle:Display(State)
                pcall(function()
                    ...
                    Label.Font = State and Enum.Font.GothamBold or Enum.Font.Gotham
                    CheckImage.ImageTransparency = State and 0 or 1
                end)
                if State then _startKbGrad() else _stopKbGrad() end
            end
            ...
            function KeybindsToggle:SetVisibility(Visibility)
                pcall(function() Holder.Visible = Visibility end)
            end
        ...
        function KeyPicker:Update()
            ...
            -- Official escape hatch for hosts that want this keybind row hidden
            -- in the Keybinds menu while the linked feature is off, instead of
            -- always showing every registered keybind regardless of feature
            -- state. Set KeyPicker.ForceHidden = true/false and call :Update()
            -- (or rely on the next Update() this function already gets called
            -- from elsewhere) to apply -- no external polling/signal-watching
            -- needed to fight this function's own SetVisibility(true) below.
            if KeyPicker.ForceHidden then
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function InitEvents(Button)
            ...
            Button.Base.MouseButton1Click:Connect(function()
                ...
                local _rp = Instance.new("Frame")
                _rp.AnchorPoint = Vector2.new(0.5, 0.5)
                _rp.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                _rp.BackgroundTransparency = 0.82
                _rp.BorderSizePixel = 0
                _rp.Size = UDim2.fromOffset(0, 0)
                _rp.Position = UDim2.fromScale(0.5, 0.5)
                _rp.ZIndex = Button.Base.ZIndex + 5
                Instance.new("UICorner", _rp).CornerRadius = UDim.new(1, 0)
                _rp.Parent = Button.Base
                local _sz = math.max(Button.Base.AbsoluteSize.X, Button.Base.AbsoluteSize.Y) * 2.5
                TweenService:Create(_rp, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    Size = UDim2.fromOffset(_sz, _sz),
                    BackgroundTransparency = 1,
                }):Play()
                task.delay(0.4, function() _rp:Destroy() end)
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        function Dropdown:Display()
            ...
            if Info.Multi then
                ...
                for _, Value in Dropdown.Values do
                    if Dropdown.Value[Value] then
                        ...
                        local _raw = tostring(Value)
                        local _sep = _first and "" or ", "
                        if _vis + #_sep + #_raw > 25 then
                            Str = Str .. "..."
                            break
                        end
                        _vis = _vis + #_sep + #_raw
                        Str = Str .. _sep .. (Info.FormatDisplayValue and tostring(Info.FormatDisplayValue(Value)) or _raw)
                        _first = false
                    end
                end
            else
            ...
            if not Info.Multi and #Str > 25 then
                Str = Str:sub(1, 22) .. "..."
            end
            ...
            for _, Value in Values do
                ...
                if not IsDisabled then
                    Button.MouseButton1Click:Connect(function()
                        ...
                        _rp.Size = UDim2.fromOffset(0, 0)
                        _rp.Position = UDim2.fromScale(0.5, 0.5)
                        _rp.ZIndex = Container.ZIndex + 5
                        Instance.new("UICorner", _rp).CornerRadius = UDim.new(1, 0)
                        _rp.Parent = Container
                        local _sz = math.max(Container.AbsoluteSize.X, Container.AbsoluteSize.Y) * 2.5
                        TweenService:Create(_rp, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                            Size = UDim2.fromOffset(_sz, _sz),
                            BackgroundTransparency = 1,
                        }):Play()
...
function Library:Notify(...)
    ...
    local Holder = New("Frame", {
        AutomaticSize = Enum.AutomaticSize.Y,
        BackgroundColor3 = "MainColor",
        Position = (Library.NotifySide or "Right"):lower() == "left" and UDim2.new(-1, -8, 0, -2) or UDim2.new(1, 8, 0, -2),
        Size = UDim2.fromScale(1, 1),
        ZIndex = 5,
    ...
    function Data:Resize()
        ...
        -- NotificationArea is a fixed 300px-wide container, but Frames don't clip
        -- their children by default -- FakeBackground/Holder below are sized from
        -- the text's own natural bounds, not from NotificationArea's width. The only
        -- thing that was forcing titles/descriptions to wrap was capping GetTextBounds
        -- to NotificationArea's width here. Use a generous bound instead so normal
        -- single-line text never wraps, while still containing genuinely long text.
        local MaxTextWidth = 560

        if Title then
            local X, Y =
                Library:GetTextBounds(Title.Text, Title.FontFace, Title.TextSize, MaxTextWidth - ExtraWidth - IconWidth)
            local singleLineH = math.ceil(Title.TextSize * 1.4)
            Y = math.min(Y, singleLineH)
            Title.Size = UDim2.fromOffset(X + 2, Y)
            TitleX = X + 2 + IconWidth
            TitleContainer.Size = UDim2.fromOffset(TitleX, math.max(Y, IconLabel and 16 or 0))
        end

        if Desc then
            local X, Y =
                Library:GetTextBounds(Desc.Text, Desc.FontFace, Desc.TextSize, MaxTextWidth - ExtraWidth)
            Desc.Size = UDim2.fromOffset(X + 2, Y)
            DescX = X + 2
        end
...
function Library:CreateWindow(WindowInfo)
    ...
    local _toggleAnimTween = nil
    local MainFrameWrapper
    local MainFrameWrapperScale
    local MainFrame
    local DividerLine
    local TitleHolder
    local WindowTitle
    ...
    do
        ...
        Library:AddGlow(Library.KeybindFrame)

        local _wrapW = WindowInfo.Size.X.Offset
        local _wrapH = WindowInfo.Size.Y.Offset
        local _wrapPos
        if WindowInfo.Center then
            _wrapPos = UDim2.fromScale(0.5, 0.5)
        else
            _wrapPos = UDim2.new(
                WindowInfo.Position.X.Scale,
                WindowInfo.Position.X.Offset + _wrapW / 2,
                WindowInfo.Position.Y.Scale,
                WindowInfo.Position.Y.Offset + _wrapH / 2
            )
        end
        MainFrameWrapper = New("Frame", {
            Name = "MainWrapper",
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundTransparency = 1,
            Position = _wrapPos,
        ...
        MainFrameWrapperScale = New("UIScale", {
            Scale = 1,
            Parent = MainFrameWrapper,
        })
        ...
        Library:MakeDraggable(MainFrameWrapper, TopBar, false, true)
    ...
    function Window:AddTab(...)
        ...
        function Tab:Hide()
            ...
            if TabSelStroke then
                TweenService:Create(TabSelStroke, Library.TweenInfo, {
                    Transparency = 1,
                }):Play()
            end
            TweenService:Create(TabLabel, Library.TweenInfo, {
        ...
        TabButton.MouseButton1Click:Connect(function()
            ...
            _rp.Size = UDim2.fromOffset(0, 0)
            _rp.Position = UDim2.fromScale(0.5, 0.5)
            _rp.ZIndex = TabButton.ZIndex + 5
            Instance.new("UICorner", _rp).CornerRadius = UDim.new(1, 0)
            _rp.Parent = TabButton
            local _sz = math.max(TabButton.AbsoluteSize.X, TabButton.AbsoluteSize.Y) * 2.5
            TweenService:Create(_rp, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Size = UDim2.fromOffset(_sz, _sz),
                BackgroundTransparency = 1,
            }):Play()
    ...
    -- Obsidian-style toggle fade (ported from upstream deividcomsono/Obsidian): tween the transparency
    -- of every MainFrame descendant on open/close (0.35s Quad Out), caching each element's visible value.
    local _tvlTransCache = {}
    local _tvlWindowAnimInfo = TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local _tvlWindowAnimInfoClose = TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    local _tvlGuiProps = { "BackgroundTransparency" }
    local _tvlImgProps = { "BackgroundTransparency", "ImageTransparency" }
    local _tvlTxtProps = { "BackgroundTransparency", "TextTransparency" }
    local _tvlStrokeProps = { "Transparency" }
    local function _tvlFadeInstance(Desc, Props)
        local cache = _tvlTransCache[Desc]
        if not cache then
            cache = {}
            _tvlTransCache[Desc] = cache
        end
        for _, Prop in Props do
            ...
            if cache[Prop] ~= nil and cache[Prop] ~= 1 then
                TweenService:Create(Desc, Library.Toggled and _tvlWindowAnimInfo or _tvlWindowAnimInfoClose, { [Prop] = Library.Toggled and cache[Prop] or 1 }):Play()
            end
        end
    end
    ...
    function Window:Toggle(Value: boolean?)
        ...
        do
            -- Obsidian fade toggle. Keep the wrapper at full scale (the old scale-pop is retired); the
            -- transparency fade is driven per-descendant by _tvlFadeInstance, with a pcall fallback so the
            -- menu still opens/closes even if a fade tween errors.
            if Library.Toggled then
                pcall(function() MainFrameWrapperScale.Scale = 1 end)
                MainFrameWrapper.Visible = true
            end
            local _fadeOk = pcall(function()
                for _, Inst in MainFrame:GetDescendants() do
                    if Inst:IsA("GuiObject") then
                        local cn = Inst.ClassName
                        if cn == "ImageLabel" or cn == "ImageButton" then
                            _tvlFadeInstance(Inst, _tvlImgProps)
                        elseif cn == "TextLabel" or cn == "TextBox" or cn == "TextButton" then
                            _tvlFadeInstance(Inst, _tvlTxtProps)
                        else
                            _tvlFadeInstance(Inst, _tvlGuiProps)
                        end
                    elseif Inst.ClassName == "UIStroke" then
                        _tvlFadeInstance(Inst, _tvlStrokeProps)
                    end
                end
                _tvlFadeInstance(MainFrame, _tvlGuiProps)
            end)
            if not Library.Toggled then
                if _fadeOk then
                    task.delay(_tvlWindowAnimInfoClose.Time, function()
                        MainFrameWrapper.Visible = Library.Toggled
                    end)
                else
                    MainFrameWrapper.Visible = false
                end
            end
        end
...
function Library:CreateLoading(LoadingInfo)
    ...
    Library:MakeDraggable(MainFrameWrapper, TopBar, true, true)
    ...
    function Loading:SetFooterButtons(Buttons)
        assert(typeof(Buttons) == "table", "Buttons must be a table")
        if SliderBar then SliderBar.Visible = false end
        if Loading._FooterRow then pcall(function() Loading._FooterRow:Destroy() end) end
        ...
        Loading._FooterRow = FooterRow
        for _, Info in ipairs(Buttons) do
            local IsLeft = Info.Align == "Left"
            local TextBtn = New("TextButton", {
                BackgroundColor3 = "MainColor",
                BorderColor3 = "OutlineColor",
                AutoButtonColor = false,
                ...
                Position = IsLeft and UDim2.new(0, 0, 0.5, 0) or UDim2.new(1, 0, 0.5, 0),
            ...
            local LabelX = Library:GetTextBounds(BtnLabel.Text, Library.Scheme.Font, 14, 300)
            TextBtn.Size = UDim2.fromOffset(LabelX + 30, 32)
            local ActiveColor = Library.Scheme.MainColor
            local HoverColor = Library:GetBetterColor(ActiveColor, 10)
            TextBtn.MouseEnter:Connect(function()
            ...
            TextBtn.MouseButton1Click:Connect(function()
                if Info.Callback then Info.Callback(Loading) end
            end)
        end
        Loading:RecalculateLoadingHeight()
        Loading:UpdateLayout()
